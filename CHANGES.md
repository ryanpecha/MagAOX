...
| `libMagAOX/logger/types/telem_sparkleclock.hpp` | Two defects: `eventCode` was `TELEM_DMSPECK` (copy-paste error — the type identified itself as a different telemetry stream), now `TELEM_SPARKLECLOCK`; and `msgString` iterated `fbs->separations()` without a null check, crashing on a message serialized without that vector. Now guarded. |
