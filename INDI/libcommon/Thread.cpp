...
Thread::Thread()
    : m_idThis( 0 ), m_uiInterval( 1000 ), m_tState( Idle ), m_oStop( true ), m_oOneShot( false ),
      m_oIsRunning( false ), m_oIsPaused( false ), m_psocTrigger( NULL )
{
...
const Thread &Thread::operator=( const Thread &copy )
{
    if( &copy != this )
    {
        ...
        m_oOneShot.store( copy.m_oOneShot.load() );
        m_idThis = 0;
        m_oStop.store( true );
        m_oIsRunning.store( false );
        m_oIsPaused.store( false );
        m_psocTrigger = NULL;
        m_tState      = Idle;
...
Thread::Thread( const Thread &copy )
    : m_idThis( 0 ), m_uiInterval( copy.m_uiInterval ), m_tState( Idle ), m_oStop( true ),
      m_oOneShot( copy.m_oOneShot.load() ), m_oIsRunning( false ), m_oIsPaused( false ), m_psocTrigger( NULL )
{
    // We are not running in 'execute' yet.
    m_mutReady.lock();
...
void Thread::setupPipe()
{
    ...
    if( ::pipe( m_pfdPipe ) == 0 )
    {
        ...
        {
            int iFlags = 0;
            // fcntl F_GETFL/F_SETFL on a just-created pipe fd cannot fail, so these
            // two cleanup blocks are unreachable defensive code.
...
int Thread::nanosleep( const unsigned int &uiSeconds, const unsigned int &uiNanos )
{
    int nErr = ErrNone;
    ...
    while( m_oStop.load() == false )
    {
        ...
        if( m_oIsPaused.load() == true )
        {
            ...
            if( FD_ISSET( m_pfdPipe[0], &fdsRead ) )
            {
                ...
                if( rv < 0 ) // select just reported the fd readable, so the read cannot fail
                    std::cerr << __FILE__ << " " << __LINE__ << " " << strerror( errno ) << "\n"; // LCOV_EXCL_LINE
            }
        }
        ...
        m_oStop.store( m_oOneShot.load() || m_oStop.load() );
    }
...
int Thread::start( const int &iCpuAffinity, const ScheduleType &tSchedule )
{
...
#ifdef WIN32
        DWORD idDummy;
        m_idThis = CreateThread( NULL, 0, Thread::threadFuncWin, this, 0, &idDummy );
