 ...
 * \returns -1 on error
 */
inline int addTextElement( pcf::IndiProperty &prop,      ///< [out] the property to which to add the elemtn
...
int addNumberElement( pcf::IndiProperty &prop,      ///< [out] the property to which to add the elemtn
                      ...
                      const std::string &format,    ///< [in] the _ value for the elements, applied to both target and
                                                    ///< current.  Set to "" to use the MagAO-X standard for type.
                      const std::string &label = "" ///< [in] [optional] the GUI label suggestion for this property
)
{
                      ...
                      pcf::IndiProperty::PropertyStateType newState = pcf::IndiProperty::Ok )
{
    ...
    try
    {
        ...
        for( n = 0; n < els.size() && changed != true; ++n )
        {
            ...
            if( p[els[n]].getValue() != ssValue.str() )
                changed = true;
        }
 ...
 * \tparam elVecT the type of the element names, should be either std::string or const char *
 * \tparam indiDriverT the type of the INDI driver class
 */
template <typename T, typename elVecT, class indiDriverT>
void updatesIfChanged( pcf::IndiProperty    &p,   ///< [in/out] The property containing the element to possibly update
                       const elVecT         &els, ///< [in] The element names
                       const std::vector<T> &newVals,    ///< [in] the new values
                       indiDriverT          *indiDriver, ///< [in] the MagAOX INDI driver to use
                       pcf::IndiProperty::PropertyStateType newState = pcf::IndiProperty::Ok )
{
...
/// Update the elements of an INDI propery, but only if there has been a change in at least one.
/** Specialization for const char * element names.
 *
 * Only sends the set property message if at least one of the new values is different, or if the state has changed.
 *
 * \tparam T the type of the values
...
template <typename T, class indiDriverT>
void updatesIfChanged( pcf::IndiProperty &p, ///< [in/out] The property containing the element to possibly update
                       const std::vector<const char *>     &els,        ///< [in] The element names
                       const std::vector<T>                &newVals,    ///< [in] the new values
                       indiDriverT                         *indiDriver, ///< [in] the MagAOX INDI driver to use
                       pcf::IndiProperty::PropertyStateType newState = pcf::IndiProperty::Ok )
{
    updatesIfChanged<T, std::vector<const char *>, indiDriverT>( p, els, newVals, indiDriver, newState );
}

/// Update the elements of an INDI propery, but only if there has been a change in at least one.
/** Specialization for std::string element names.
 *
 * Only sends the set property message if at least one of the new values is different, or if the state has changed.
 *
 * \tparam T the type of the values
...
void updatesIfChanged( pcf::IndiProperty &p, ///< [in/out] The property containing the element to possibly update
                       ...
                       pcf::IndiProperty::PropertyStateType newState = pcf::IndiProperty::Ok )
{
    updatesIfChanged<T, std::vector<std::string>, indiDriverT>( p, els, newVals, indiDriver, newState );
}
 ...
 */
template <class indiDriverT>
void updateSelectionSwitchIfChanged(
 ...
 */
inline int parseIndiKey( std::string       &devName,  ///< [out] the device name
