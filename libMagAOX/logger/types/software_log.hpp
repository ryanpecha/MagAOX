...
/** Such logs are used to log software status, warnings, and errors. Does not have defaultLevel, so this can not be used
 * as a log type in logger.
 *
...
struct software_log : public flatbuffer_log
{
    ...
    struct messageT : public fbMessage
    {
        ...
        messageT( const char    *file,      ///< [in] The file of the error, should always be \c \_\_FILE\_\_
                  ...
                  const int32_t  errnoCode, ///< [in] The errno code at the time of the log entry. Only errno should be
        ...
        // constructors using source location

        messageT( const int32_t errnoCode,                 /**< [in] The errno code at the time of the log entry. Only
                                                                     errno should be passed here, so strerror can be used later.*/
                  const int32_t otherCode,                 /**< [in] Some other error code, such as a return value or
                                                                     library code. */
                  const char                 *explanation, /**< [in] explanatory text about the software event*/
                  ...
                  = std::source_location::current() )
        {
            ...
            auto gs = CreateSoftware_log_fb( builder, _file, loc.line(), errnoCode, 0, 0 );
            builder.Finish( gs );
        }
                  ...
                  = std::source_location::current() )
        {
            ...
            auto gs = CreateSoftware_log_fb( builder, _file, loc.line(), errnoCode, 0, _expl );
            builder.Finish( gs );
        }
        ...
        /// C'tor with no codes, just the explanation.
        messageT( const char                 *explanation, /**< [in] explanatory text about the software event */
                  const std::source_location &loc          /**< [in] [opt] source location */
                  = std::source_location::current() )
        {
            auto _file = builder.CreateString( loc.file_name() );
                  ...
                  = std::source_location::current() )
        {
            ...
            auto gs = CreateSoftware_log_fb( builder, _file, loc.line(), 0, 0, 0 );
            builder.Finish( gs );
        }
    };
...
/** This should only be used for a system-wide emergency requiring operator or automatic shutdown.  Not for a process
 * specific problem.
 * \includedoc sw_logs.dox.inc
...
/** This should only be used for a system-wide emergency requiring operator or automatic action.  Not for a process
 * specific problem.
 * \includedoc sw_logs.dox.inc
 * \ingroup logger_types
 */
