...
struct telem_teldata : public flatbuffer_log
{
    ...
    struct messageT : public fbMessage
    {
        ...
        {
            auto fp = CreateTelem_teldata_fb(
                builder, roi, tracking, guiding, slewing, guiderMoving, az, zd, pa, domeAz, domeStat );
            builder.Finish( fp );
        }
    };
    ...
    static logMetaDetail getAccessor( const std::string &member /**< [in] the name of the member */ )
    {
        if( member == "roi" )
            return logMetaDetail(
                { "ROI", logMeta::valTypes::Int, logMeta::metaTypes::State, reinterpret_cast<void *>( &roi ) } );
        ...
        else if( member == "guiding" )
            return logMetaDetail( { "GUIDING",
                                    logMeta::valTypes::Int,
                                    logMeta::metaTypes::State,
                                    reinterpret_cast<void *>( &guiding ) } );
        else if( member == "slewing" )
            return logMetaDetail( { "SLEWING",
                                    logMeta::valTypes::Int,
                                    logMeta::metaTypes::State,
                                    reinterpret_cast<void *>( &slewing ) } );
        else if( member == "guiderMoving" )
            return logMetaDetail( { "GUIDER MOVING",
                                    logMeta::valTypes::Int,
                                    logMeta::metaTypes::State,
                                    reinterpret_cast<void *>( &guiderMoving ) } );
        else if( member == "az" )
            return logMetaDetail( { "AZ",
                                    logMeta::valTypes::Double,
                                    logMeta::metaTypes::Continuous,
                                    reinterpret_cast<void *>( &az ),
                                    false } );
        else if( member == "zd" )
            return logMetaDetail( { "ZD",
                                    logMeta::valTypes::Double,
                                    logMeta::metaTypes::Continuous,
                                    reinterpret_cast<void *>( &zd ),
                                    false } );
        else if( member == "pa" )
            return logMetaDetail( { "PARANG",
                                    logMeta::valTypes::Double,
                                    logMeta::metaTypes::Continuous_Angle,
                                    reinterpret_cast<void *>( &pa ),
                                    false } );
        else if( member == "domeAz" )
            return logMetaDetail( { "DOME AZ",
                                    logMeta::valTypes::Double,
                                    logMeta::metaTypes::Continuous,
                                    reinterpret_cast<void *>( &domeAz ) } );
        else if( member == "domeStat" )
            return logMetaDetail( { "DOME STATUS",
                                    logMeta::valTypes::Int,
                                    logMeta::metaTypes::State,
                                    reinterpret_cast<void *>( &domeStat ) } );
        else
        {
            std::cerr << "No member " << member << " in telem_teldata\n";
            return logMetaDetail();
        }
    }
