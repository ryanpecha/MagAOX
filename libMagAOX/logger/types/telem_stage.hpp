...
struct telem_stage : public flatbuffer_log
{
   ...
   static std::string presetName( void * msgBuffer )
   {
      ...
      else return std::string(); // LCOV_EXCL_LINE the only constructor always sets presetName so the null check cannot fail
   }
   ...
   static logMetaDetail getAccessor( const std::string & member /**< [in] the name of the member */ )
   {
      ...
      {
         std::cerr << "No member " << member << " in telem_stage\n";
